    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form>
              <div class="invalid-feedback" v-if="userNotFound">
                  User not Found
              </div>
          ...
          <div class="card mb-3">
            ...
            <div class="card-body">
              ...
              <p class="card-text" v-for="comment in post.coments" >
                  <span @click="commentUser = comment.user; commentId = comment.id" style="cursor:pointer;">
                      {{comment.text + ' - '}}<small class="text-muted">{{comment.user.personaname}}</small>
                  </span>
                  <br>
                  <small style="margin-left: 10px; font-size: 0.7em; cursor:pointer;" class="heart" @click="addCommentLike(comment.id)">
                      <svg class="bi bi-heart-fill" width="1em" height="1em" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                          <path fill-rule="evenodd" d="M8 2.748l-.717-.737C5.6.281 2.514.878 1.4 3.053c-.523 1.023-.641 2.5.314 4.385.92 1.815 2.834 3.989 6.286 6.357 3.452-2.368 5.365-4.542 6.286-6.357.955-1.886.838-3.362.314-4.385C13.486.878 10.4.28 8.717 2.01L8 2.748zM8 15C-7.333 4.868 3.279-3.04 7.824 1.143c.06.055.119.112.176.171a3.12 3.12 0 01.176-.17C12.72-3.042 23.333 4.867 8 15z" clip-rule="evenodd"/>
                      </svg>
                      {{comment.likes}}
                      ...
                      <span v-for="child in comment.children" style="display:block;">
                          <span>
                                --- {{child.text + ' - '}}<small class="text-muted">{{child.user.personaname}}</small>
                          </span>


                          <small style="margin-left: 10px; font-size: 0.7em; cursor:pointer;" class="heart" @click="addCommentLike(child.id)">
                              <svg class="bi bi-heart-fill" width="1em" height="1em" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                  <path fill-rule="evenodd" d="M8 2.748l-.717-.737C5.6.281 2.514.878 1.4 3.053c-.523 1.023-.641 2.5.314 4.385.92 1.815 2.834 3.989 6.286 6.357 3.452-2.368 5.365-4.542 6.286-6.357.955-1.886.838-3.362.314-4.385C13.486.878 10.4.28 8.717 2.01L8 2.748zM8 15C-7.333 4.868 3.279-3.04 7.824 1.143c.06.055.119.112.176.171a3.12 3.12 0 01.176-.17C12.72-3.042 23.333 4.867 8 15z" clip-rule="evenodd"/>
                              </svg>
                              {{child.likes}}
                ...
                <?php if(User_model::is_logged()): ?>
                    <form class="form-inline" v-if="commentUser !== null">
                        <span style="margin-right: 10px;">@{{commentUser.personaname}}  <span style="color: red; cursor:pointer;" title="Remove answer" @click="commentUser = null">X</span></span>
                        <div class="form-group">
                            <input type="text" class="form-control" id="addComment" v-model="commentText">
                        </div>
                        <button type="button" class="btn btn-primary" @click="createComment(post.id, commentText, commentId)">Answer comment</button>
                    </form>
